...
void CRatnaKiranReports::FinalizeLandscapeReport(BOOL add_right_footer)
{
	if(m_filename.IsEmpty())
	{
		...
		if(!lpDisp)
		{
			MessageBox(NULL, "CRatnaKiranReports::FinalizeLandscapeReport - couldn't get pagesetup.", "ERROR", MB_OK);
			return;
		}
...
void CRatnaKiranReports::FinalizePotraitReport(BOOL pricequotation)
{
	if(m_filename.IsEmpty())
	{
		...
		if(!lpDisp)
		{
			MessageBox(NULL, "CRatnaKiranReports::FinalizePotraitReport - couldn't get pagesetup.", "ERROR", MB_OK);
			return;
		}
...
void CRatnaKiranReports::FinalizeTagReport()
{
	if(m_filename.IsEmpty())
	{
		...
		if(!lpDisp)
		{
			MessageBox(NULL, "CRatnaKiranReports::FinalizeTagReport - couldn't get pagesetup.", "ERROR", MB_OK);
			return;
		}
...
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreateStocksReportActual - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
	...
	while(!m_list->IsEmpty())
	{
		...
		SProductSetNode *set = m_database->GetSet(productID);
		if(set)
			position = StockReport_Actual_AddSet(set, position, type);
		else
		{
...
void CRatnaKiranReports::StockReport_Actual_AddTitle()
{
...
void CRatnaKiranReports::StockReport_Actual_AddHeaders()
{
	...
	MergeCells(gAlpha[column], position, gAlpha[column], position + 1);
	AddTotals(column);
	FillCell(gAlpha[column++], position, "Labour (Rs.)");
...
int CRatnaKiranReports::StockReport_Actual_AddSet(SProductSetNode *set, int position, ReportType type)
{
	if(!set)
		return position;

	if(type == Sold && set->GetSoldItemCount() == 0)
		return position;
	else if(type == UnSold && set->GetUnsoldItemCount() == 0)
		return position;

	int pos = position;

	FillCell(gAlpha[0], position, common_serial);
	FillCell(gAlpha[1], position, set->m_productID);
	common_serial++;

	int index = 0;
	while(index < set->GetItemCount())
	{
		SProductItemNode *item = set->GetItemAt(index);
		if(item)
		{
			if(((type == Sold) && item->IsItemSold()) ||
			   ((type == UnSold) && !(item->IsItemSold())) ||
				type == Original)
			{
				pos = StockReport_Actual_AddItem(item,pos);
			}
		}
...
int CRatnaKiranReports::StockReport_Actual_AddItem(SProductItemNode *item, int position)
{
	if(!item)
		return position;

	CString filename;
	if(m_database->IsRootDatabase())
	{
		CString productID = item->m_productID;
		if(!m_database->GetDatabaseNameForItem(productID, &filename))
			filename = "";
	}

	SFileHeaderNode *dbHeader = m_database->GetHeader(filename);
	if(!dbHeader)
		return position;
	...
	FillCell(gAlpha[column++], position, item->GetGoldValue(item->m_use_net_weight_for_gold, TRUE, dbHeader->GetGoldRate(), 1.0), 2);

	int index = 0;
	while(index < item->GetDiamondTypeCount())
	{
		SetAlignment(gAlpha[column], position + index, RIGHTALIGN, CENTERALIGN);
		FillCell(gAlpha[column], position + index, item->GetDiamondTypeOf(index));
		FillCell(gAlpha[column + 1], position + index, item->GetDiamondWeightOf(index), 2);
		FillCell(gAlpha[column + 2], position + index, item->GetDiamondRateOf(index, 1.0));
		FillCell(gAlpha[column + 3], position + index, item->GetDiamondValueOf(index, 1.0), 2);

		index++;
	}

	/*
	if(item->GetDiamondTypeCount() > 1)
	{
		SetAlignment(gAlpha[column], position + index, RIGHTALIGN, CENTERALIGN);
		FillCell(gAlpha[column], position + index, "Total");
		FillCell(gAlpha[column + 1], position + index, item->GetDiamondWeight(), 2);
		FillCell(gAlpha[column + 2], position + index, item->GetDiamondRate(1.0));
		FillCell(gAlpha[column + 3], position + index, item->GetDiamondValue(1.0), 2);
	}
	*/
	...
	index = 0;
	while(index < item->GetStoneTypeCount())
	{
		SetAlignment(gAlpha[column], position + index, RIGHTALIGN, CENTERALIGN);
		FillCell(gAlpha[column], position + index, item->GetStoneTypeOf(index));
		FillCell(gAlpha[column + 1], position + index, item->GetStoneWeightOf(index), 2);
		FillCell(gAlpha[column + 2], position + index, item->GetStoneRateOf(index, 1.0));
		FillCell(gAlpha[column + 3], position + index, item->GetStoneValueOf(index, 1.0), 2);

		index++;
	}

	/*
	if(item->GetStoneTypeCount() > 1)
	{
		SetAlignment(gAlpha[column], position + index, RIGHTALIGN, CENTERALIGN);
		FillCell(gAlpha[column], position + index, "Total");
		FillCell(gAlpha[column + 1], position + index, item->GetStoneWeight(), 2);
		FillCell(gAlpha[column + 2], position + index, item->GetStoneRate(1.0));
		FillCell(gAlpha[column + 3], position + index, item->GetStoneValue(1.0), 2);
	}
	*/

	column += 4;

	FillCell(gAlpha[column++], position, item->GetPearlWeight(), 2);
	FillCell(gAlpha[column++], position, item->GetPearlRate(1.0));
	FillCell(gAlpha[column++], position, item->GetPearlValue(1.0), 2);
	FillCell(gAlpha[column++], position, item->GetOtherWeight(), 3);
	FillCell(gAlpha[column++], position, item->GetLabourCost(1.0), 2);
	FillCell(gAlpha[column++], position, item->GetTotalValue(item->m_use_net_weight_for_gold, TRUE, dbHeader->GetGoldRate(), 1.0), 2);

	int newpos;
	int maxval = max(item->GetDiamondTypeCount(), item->GetStoneTypeCount());
	maxval = max(1, maxval);
...
BOOL CRatnaKiranReports::CreateStocksReportMarked(ReportType type)
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreateStocksReportMarked - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
	...
	while(!m_list->IsEmpty())
	{
		...
		SProductSetNode *set = m_database->GetSet(productID);
		if(set)
			position = StockReport_Marked_AddSet(set, position, type);
		else
		{
			CString errstr = "Could not get set details for " + productID + ".";
			MessageBox(NULL, errstr, "ERROR", MB_OK);
...
void CRatnaKiranReports::StockReport_Marked_AddTitle()
{
	SetBold(gAlpha[0], 1, gAlpha[m_last_column], 1);
	MergeCells(gAlpha[0], 1, gAlpha[m_last_column], 1);
...
//Returns the position where the next entry can be made
int CRatnaKiranReports::StockReport_Marked_AddSet(SProductSetNode *set, int position, ReportType type)
{
	if(!set)
		return position;
		...
		if(item)
		{
			if(((type == Sold) && item->IsItemSold()) ||
			   ((type == UnSold) && !(item->IsItemSold())) ||
				type == Original)
			{
				pos = StockReport_Marked_AddItem(item,pos);
			}
		}
...
int CRatnaKiranReports::StockReport_Marked_AddItem(SProductItemNode *item, int position)
{
	if(!item)
		return position;
	...
	while(index < item->GetDiamondTypeCount())
	{
		...
		FillCell(gAlpha[column], position + index, item->GetDiamondTypeOf(index));
		FillCell(gAlpha[column + 1], position + index, item->GetDiamondWeightOf(index), 2);
		FillCell(gAlpha[column + 2], position + index, item->GetMarkedDiamondRateOf(index, dbHeader->GetGoldRate(),1.0));
		FillCell(gAlpha[column + 3], position + index, item->GetMarkedDiamondValueOf(index, dbHeader->GetGoldRate(),1.0), 2);
	...
	if(item->GetDiamondTypeCount() > 1)
	{
		...
		FillCell(gAlpha[column], position + index, "Total");
		FillCell(gAlpha[column + 1], position + index, item->GetDiamondWeight(), 2);
		FillCell(gAlpha[column + 2], position + index, item->GetMarkedDiamondRate(dbHeader->GetGoldRate(),1.0));
		FillCell(gAlpha[column + 3], position + index, item->GetMarkedDiamondValue(dbHeader->GetGoldRate(),1.0), 2);
	}
	*/
	...
	while(index < item->GetStoneTypeCount())
	{
		...
		FillCell(gAlpha[column], position + index, item->GetStoneTypeOf(index));
		FillCell(gAlpha[column + 1], position + index, item->GetStoneWeightOf(index), 2);
		FillCell(gAlpha[column + 2], position + index, item->GetMarkedStoneRateOf(index, dbHeader->GetGoldRate(),1.0));
		FillCell(gAlpha[column + 3], position + index, item->GetMarkedStoneValueOf(index, dbHeader->GetGoldRate(),1.0), 2);
	...
	if(item->GetStoneTypeCount() > 1)
	{
		...
		FillCell(gAlpha[column], position + index, "Total");
		FillCell(gAlpha[column + 1], position + index, item->GetStoneWeight(), 2);
		FillCell(gAlpha[column + 2], position + index, item->GetMarkedStoneRate(dbHeader->GetGoldRate(),1.0));
		FillCell(gAlpha[column + 3], position + index, item->GetMarkedStoneValue(dbHeader->GetGoldRate(),1.0), 2);
	}
	*/
	...
	FillCell(gAlpha[column++], position, item->GetPearlWeight(), 2);
	FillCell(gAlpha[column++], position, item->GetMarkedPearlRate(dbHeader->GetGoldRate(),1.0));
	FillCell(gAlpha[column++], position, item->GetMarkedPearlValue(dbHeader->GetGoldRate(),1.0), 2);
	FillCell(gAlpha[column++], position, item->GetOtherWeight(), 3);
	FillCell(gAlpha[column++], position, item->GetMarkedLabourCost(dbHeader->GetGoldRate(),1.0), 2);
	FillCell(gAlpha[column++], position, item->GetTotalMarkedValue(item->m_use_net_weight_for_gold, TRUE, dbHeader->GetGoldRate(),1.0), 2);
...
BOOL CRatnaKiranReports::CreateStockPriceQuotation()
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreateStockPriceQuotation - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
	...
	while(!m_list->IsEmpty())
	{
		...
		if(set)
		{
			...
			position = StockPriceQuotation_AddSet(set, position, orientation);
...
void CRatnaKiranReports::StockPriceQuotation_AddTitle()
{
	SetBold(gAlpha[0], 1, gAlpha[m_last_column], 1);
	MergeCells(gAlpha[0], 1, gAlpha[m_last_column], 1);
	FillCell(gAlpha[0], 1, "Stock Price Quotation");
	SetAlignment(gAlpha[0], 1, CENTERALIGN, CENTERALIGN);
	SetFontSize(gAlpha[0], 1, gAlpha[m_last_column], 1, 10);
	SetUnderline(gAlpha[0], 1, gAlpha[m_last_column], 1);
}
...
void CRatnaKiranReports::StockPriceQuotation_AddHeaders()
{
	...
	MergeCells(gAlpha[column], position, gAlpha[column], position + 1);
	SetWidth(gAlpha[column], gAlpha[column], 11);
	FillCell(gAlpha[column++], position, "Item ID");

	MergeCells(gAlpha[column], position, gAlpha[column], position + 1);
	SetWidth(gAlpha[column], gAlpha[column], 16);
	FillCell(gAlpha[column++], position, "Item Desc.");

	MergeCells(gAlpha[column], position, gAlpha[column], position + 1);
	AddTotals(column, FALSE);
	SetWidth(gAlpha[column], gAlpha[column], 10);
...
#if USE_TWO_COLUMNS
	SetWidth(gAlpha[price_quotation_number_of_columns], gAlpha[price_quotation_number_of_columns], 2);
...
int CRatnaKiranReports::StockPriceQuotation_AddSet(SProductSetNode *set, int position, int orientation)
{
	if(!set)
		return position;
	...
	while(index < set->GetItemCount())
	{
		...
		if(item)
		{
			...
			{
				pos = StockPriceQuotation_AddItem(item, pos, orientation);
			}
		}
...
int CRatnaKiranReports::StockPriceQuotation_AddItem(SProductItemNode *item, int position, int orientation)
{
	if(!item)
		return position;
	...
	double gold = dbHeader->GetGoldRate();
	double pricequote;
	CString price = dbHeader->m_price_quote;
	...
	FillCell(gAlpha[column++], position, item->m_productID);
	FillCell(gAlpha[column++], position, item->m_description);
...
BOOL CRatnaKiranReports::CreateStockChecklist()
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreateStockChecklist - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
	...
	while(!m_list->IsEmpty())
	{
		...
		SProductSetNode *set = m_database->GetSet(productID);
		if(set)
			position = StockChecklist_AddSet(set,position);
		else
		{
			CString errstr = "Could not get set details for " + productID + ".";
			MessageBox(NULL, errstr, "ERROR", MB_OK);
...
void CRatnaKiranReports::StockChecklist_AddTitle()
{
	SetBold(gAlpha[0], 1, gAlpha[m_last_column], 1);
	MergeCells(gAlpha[0], 1, gAlpha[m_last_column], 1);
	FillCell(gAlpha[0], 1, "CHECKLIST");
	SetAlignment(gAlpha[0], 1, CENTERALIGN,CENTERALIGN);
	SetFontSize(gAlpha[0], 1, gAlpha[m_last_column], 1, 14);
	SetUnderline(gAlpha[0], 1, gAlpha[m_last_column], 1);
}
...
//Returns the position where the next entry can be made
int CRatnaKiranReports::StockChecklist_AddSet(SProductSetNode *set, int position)
{
	if(!set)
		return position;
	...
	while(index < set->GetItemCount())
	{
		SProductItemNode *item = set->GetItemAt(index);
		if(item)
			pos = StockChecklist_AddItem(item,pos);
...
int CRatnaKiranReports::StockChecklist_AddItem(SProductItemNode *item, int position)
{
	if(!item)
		return position;
...
BOOL CRatnaKiranReports::CreateStockTags(int row, int column)
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreateStockTags - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
	...
	while(!m_list->IsEmpty())
	{
		...
		if(set)
		{
			...
			while(index < set->GetItemCount())
			{
				...
				if(item)
				{
					StockTags_AddItem(item, (2 * itemnumber), position);
					itemnumber++;
...
void CRatnaKiranReports::StockTags_AddItem(SProductItemNode *item, int column, int row)
{
	if(!item)
		return;
...

	{
		...
		//GOLD VALUE
		temp.Format("%d",item->GetGoldValue(item->m_use_net_weight_for_gold, TRUE, dbHeader->GetGoldRate(), 1.0) + item->GetMarkedLabourCost(gGoldRate, 1.0));
		firstside = firstside + "MM:" + temp;

		//LABOUR COST
		temp.Format("%d",item->GetMarkedLabourCost(dbHeader->GetGoldRate(), 1.0));
		firstside = firstside + " / " + temp + "\n";

		//TOTAL VALUE
		temp.Format("%d", item->GetTotalMarkedValue(item->m_use_net_weight_for_gold, TRUE, dbHeader->GetGoldRate(), 1.0));
...

	{
		...
		//DIAMOND WEIGHT
		int count = 0;
		while(count < item->GetDiamondTypeCount())
		{
			CString type, rate, weight;
			type = item->GetDiamondTypeOf(count);
			weight.Format("%.02f", item->GetDiamondWeightOf(count));
			rate.Format("%d", item->GetMarkedDiamondRateOf(count, dbHeader->GetGoldRate(), 1.0));
		...
		//STONE WEIGHT
		count = 0;
		while(count < item->GetStoneTypeCount())
		{
			CString type, rate, weight;
			type = item->GetStoneTypeOf(count);
			weight.Format("%.02f", item->GetStoneWeightOf(count));
			rate.Format("%d", item->GetMarkedStoneRateOf(count, dbHeader->GetGoldRate(), 1.0));
			secondside = secondside + type + " -" + weight + " -" + rate + "\n";
			count++;
		}
		...
		//PEARL WEIGHT
		{
			CString type, rate, weight;
			type = "P";
			weight.Format("%.02f", item->GetPearlWeight());
			rate.Format("%d", item->GetMarkedPearlRate(dbHeader->GetGoldRate(), 1.0));

			double wt = item->GetPearlWeight() + 0.5;
			if((int)wt != 0)
				secondside = secondside + type + " -" + weight + " -" + rate + "\n";
		}
	}
...
BOOL CRatnaKiranReports::CreateKundanPriceQuotation()
{
	//salil
	return TRUE;
}

BOOL CRatnaKiranReports::CreateKundanChecklist()
{
	//salil
	return TRUE;
}
...
SProductItemNode *CRatnaKiranReports::GetLowestSerialNumberItem(CList<SProductItemNode *, SProductItemNode *> *list)
{
	if(!list)
		return NULL;

	SProductItemNode *ret_item = NULL;
	...
	while(pos)
	{
		SProductItemNode *tempitem = list->GetAt(pos);
...
SProductSetNode *CRatnaKiranReports::GetLowestSerialNumberSet(CList<SProductSetNode *, SProductSetNode *> *list)
{
	if(!list)
		return NULL;

	SProductSetNode *ret_set = NULL;
	POSITION ret_pos = NULL;
	int min_serial = -1;
	...
	while(pos)
	{
		SProductSetNode *tempset = list->GetAt(pos);
		if(tempset)
		{
			SProductItemNode *tempitem = tempset->GetItemAt(0);
			if(tempitem)
			{
				if((min_serial == -1) || (tempitem->m_serial_number < min_serial))
				{
...
void CRatnaKiranReports::CalculateExportValues(double *gold_rate, double *currency_conversion, double *labourvalue, double *pearlvalue, 
											   double *diamondvalue, double *stonevalue, double *pricequote, SProductItemNode *item)
{
	...
	if(gUseNativeCurrency)
		conversion = dbHeader->m_currency_conversion;
	else
		conversion = dbHeader->m_dollar_rate;
	...
	if(labourvalue)
	{
        CString labour = dbHeader->m_exports_labour_cost;
	...
	if(pearlvalue)
	{
		CString pearl = dbHeader->m_exports_pearl_rate;
		double value = item->GetPearlValueDouble(conversion);

		if(!pearl.IsEmpty())
		{
			int rate;
			ReplaceDollarWithValue(&pearl, item->GetPearlRate(1.0));
			rate = (int)(ProcessFormula(pearl, gold, 1.0, item) + 0.5);
			value = item->GetPearlValueUsingDouble(rate, conversion);
		}
	...
	if(diamondvalue)
	{
		CString diamond = dbHeader->m_exports_diamond_rate;
		double value = item->GetDiamondValueDouble(conversion);
		...
		if(!diamond.IsEmpty())
		{
			...
			while(count < item->GetDiamondTypeCount())
			{
				int rate;
				CString str = diamond;
				ReplaceDollarWithValue(&str, item->GetDiamondRateOf(count, 1.0));
				rate = (int)(ProcessFormula(str, gold, 1.0, item) + 0.5);
				value += item->GetDiamondValueOfUsingDouble(count, rate, conversion);
				count++;
			}
		}
	...
	if(stonevalue)
	{
		CString stone = dbHeader->m_exports_stone_rate;
		double value = item->GetStoneValueDouble(conversion);
		...
		if(!stone.IsEmpty())
		{
			...
			while(count < item->GetStoneTypeCount())
			{
				int rate;
				CString str = stone;
				ReplaceDollarWithValue(&str, item->GetStoneRateOf(count, 1.0));
				rate = (int)(ProcessFormula(str, gold, 1.0, item) + 0.5);
				value += item->GetStoneValueOfUsingDouble(count, rate, conversion);
				count++;
			}
		}
...
BOOL CRatnaKiranReports::CreatePackageList(ReportType type)
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreatePackageList - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
	...
	m_start_row = 17;
	Packagelist_AddHeaders();
	Packagelist_AddTitle(type);
	progress1.ProgressForward();

	int position = m_start_row;
	CList<SProductItemNode *, SProductItemNode *> itemlist;
	while(!m_list->IsEmpty())
	{
		CString productID = m_list->RemoveHead();
		SProductSetNode *set = m_database->GetSet(productID);
		...
		{
			if((type == Sold && set->GetSoldItemCount() != 0) ||
			   (type == UnSold && set->GetUnsoldItemCount() != 0) ||
			   (type == Original))
			{
				int index = 0;
				while(index < set->GetItemCount())
	...
	{
		SProductItemNode *item = GetLowestSerialNumberItem(&itemlist);
		if(item)
			position = Packagelist_AddItem(item, position);
...
void CRatnaKiranReports::Packagelist_AddTitle(ReportType type)
{
	SFileHeaderNode *dbHeader = m_header;
	if(!dbHeader)
	{
		dbHeader = m_database->GetHeader();
		if(!dbHeader)
			return;
	}

	CString consignee = dbHeader->m_consignee;
	CString exporter = dbHeader->m_exporter;
	CString invoice_number = dbHeader->m_invoice_number;
	...
	if(type == Original)
		...
		FillCell(gAlpha[0], 1, "SOLD PACKING LIST");
	else if(type == UnSold)
		FillCell(gAlpha[0], 1, "UNSOLD PACKING LIST");
...
void CRatnaKiranReports::Packagelist_AddHeaders()
{
	SFileHeaderNode *dbHeader = m_header;
	if(!dbHeader)
	{
	...
	CString currency;
	if(gUseNativeCurrency)
		currency = dbHeader->m_currency_name;
	else
		currency = "USD";
	...
	MergeCells(gAlpha[column], position, gAlpha[column], position + 1);
	AddTotals(column);
	FillCell(gAlpha[column++], position, CString("Labour charge (") + currency + CString(")"));
...
int CRatnaKiranReports::Packagelist_AddItem(SProductItemNode *item, int position)
{
	if(!item)
		return position;

	double gold_rate, conversion, labourvalue, pearlvalue, diamondvalue, stonevalue;
	CalculateExportValues(&gold_rate, &conversion, &labourvalue, &pearlvalue, &diamondvalue, &stonevalue, NULL, item);
	...
	if(m_use_old_serials)
	{
		int serial = item->GetOldSerial(m_old_serial_tagname);
		if(serial == -1)
	...
	FillCell(gAlpha[column++], position, item->GetDiamondWeight(), 2);
	FillCell(gAlpha[column++], position, diamondvalue, 2);
	FillCell(gAlpha[column++], position, item->GetStoneWeight(), 2);
...
BOOL CRatnaKiranReports::CreateShortInvoice(ReportType type)
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreateShortInvoice - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
	...
	while(!m_list->IsEmpty())
	{
		...
		if(set)
		{
			...
			while(index < set->GetItemCount())
			{
				...
				{
					if((type == Original) ||
					   ((type == Sold) && item->IsItemSold()) ||
					   ((type == UnSold) && !(item->IsItemSold())))
					{
						if((item->m_serial_number >= m_serials_from) &&
						((m_serials_to == -1) || (item->m_serial_number <= m_serials_to)))
						{
							ShortInvoice_AddItem(item, &pieces, &gross_weight, &net_weight, &net_value, &diamond_weight, &diamond_value,
	...
	ShortInvoice_FillValues(pieces, gross_weight, net_weight, net_value, diamond_weight, diamond_value,
...
void CRatnaKiranReports::ShortInvoice_AddItem(SProductItemNode *item, 
											  ...
											  double *total_value)
{
	...
	*diamond_weight = *diamond_weight + RoundOff(item->GetDiamondWeight(), 2);
	*diamond_value = *diamond_value + diamondvalue;
	*stone_weight = *stone_weight + RoundOff(item->GetStoneWeight(), 2);
	*stone_value = *stone_value + stonevalue;
	*pearl_weight = *pearl_weight + RoundOff(item->GetPearlWeight(), 2);
...
void CRatnaKiranReports::ShortInvoice_CreateOutline(char *startcolumn, int startrow, char *endcolumn, int endrow, BOOL hasEntry, BOOL drawBorder)
{
	if(hasEntry)
	{
...
void CRatnaKiranReports::ShortInvoice_FillValues(int pieces, double gross_weight, 
												 double net_weight, double net_value,
												 double diamond_weight, double diamond_value,
												 double stone_weight, double stone_value,
												 double pearl_weight, double pearl_value,
												 ...
												 double total_value, ReportType type)
{
	...
	CString exporter = dbHeader->m_exporter;
	CString invoice_number = dbHeader->m_invoice_number;
	CString country = dbHeader->m_country;
	CTime systemtime = CTime::GetCurrentTime();
	...
	double gold_rate = (dbHeader->GetGoldRate() * 40.0) / (conversion * 3.0);
	...
	//GROSS WEIGHT
	ShortInvoice_CreateOutline("A", 31, "B", 32, TRUE, FALSE);
	FillCell("A", 31, "Gross Weight in Gms.");
	FillCell("A", 32, gross_weight, 3);

	//DIAMOND QUALITY
	ShortInvoice_CreateOutline("A", 33, "B", 34, TRUE, FALSE);
	FillCell("A", 33, "Diamond Quality:");
	FillCell("A", 34, "VS");

	//GR FORM No.
	ShortInvoice_CreateOutline("A", 35, "B", 36, TRUE, FALSE);
	FillCell("A", 35, "GR Form No.:");
	FillCell("A", 36, dlg.m_gr_number);

	//No. OF PIECES
	ShortInvoice_CreateOutline("A", 37, "B", 38, TRUE, FALSE);
	FillCell("A", 37, "No. Of Pcs.:");
	FillCell("A", 38, pieces);

	//CUSTOM EXCHANGE RATE
	ShortInvoice_CreateOutline("A", 40, "C", 41, TRUE, FALSE);
	...
	//VALUE OF GOLD
	ShortInvoice_CreateOutline("A", 43, "C", 44, TRUE, FALSE);
	str.Format("%.02f",gold_rate);
	str = currency + " " + str + " per 10 gms.";
	FillCell("A", 43, "Value of Gold:");
	FillCell("A", 44, str);
	ShortInvoice_CreateOutline("A", 45, "C", 45, FALSE, FALSE);
...
#if 0
	//GROSS WEIGHT
	ShortInvoice_CreateOutline("F", 31, "G", 31, FALSE, FALSE);
	...
	//NET WEIGHT
	ShortInvoice_CreateOutline("F", 33, "G", 33, FALSE, FALSE);
	...
	//DIAMOND WEIGHT
	ShortInvoice_CreateOutline("F", 35, "G", 35, FALSE, FALSE);
	...
	FillCell("I", 35, SProductItemNode::GramsFromCaret(diamond_weight), 3);
	...
	//STONE WEIGHT
	ShortInvoice_CreateOutline("F", 37, "G", 37, FALSE, FALSE);
	...
	FillCell("I", 37, SProductItemNode::GramsFromCaret(stone_weight), 3);
	...
	//PEARL WEIGHT
	ShortInvoice_CreateOutline("F", 39, "G", 39, FALSE, FALSE);
	...
	FillCell("I", 39, SProductItemNode::GramsFromCaret(pearl_weight), 3);
	...
	//OTHER WEIGHT
	ShortInvoice_CreateOutline("F", 41, "G", 41, FALSE, FALSE);
	...
	//VALUE ADDITION
	ShortInvoice_CreateOutline("F", 43, "G", 43, FALSE, FALSE);
	...
	//FOB VALUE
	ShortInvoice_CreateOutline("F", 45, "G", 45, FALSE, FALSE);
	...
	//AMOUNT CHARGEABLE
	ShortInvoice_CreateOutline("A", 47, "K", 48, TRUE, TRUE);
	FillCell("A", 47, "Amount Chargeable: ");

	//WASTAGE ON GOLD
	ShortInvoice_CreateOutline("A", 50, "E", 52, FALSE, FALSE);
	FillCell("A", 50, "Wastage on gold to be taken as per chapter 4 of export import policy 2002-07.");

	//DECLARATION
	ShortInvoice_CreateOutline("A", 56, "F", 58, FALSE, FALSE);
	FillCell("A", 56, "Declaration:\nWe declare that this Invoice shows the actual price of the Goods described and that all particulars are true and correct.");

	//SIGNATURE AND DATE
	ShortInvoice_CreateOutline("G", 52, "K", 58, TRUE, TRUE);
	FillCell("G", 52, "Signature & Date");
	FillCell("G", 53, "Date - " + date);

	SetFontSize(gAlpha[0], 3, gAlpha[m_last_column], m_end_row, 8);
...
BOOL CRatnaKiranReports::CreatePhotoInvoice(ReportType type)
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreatePhotoInvoice - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
	...
	m_start_row = 17;
	PhotoInvoice_AddHeaders();
	PhotoInvoice_AddTitle(type);
	progress1.ProgressForward();
	...
	{
		CString productID = m_list->RemoveHead();
		SProductSetNode *set = m_database->GetSetWithImages(productID);
		if(set)
		{
			if((type == Sold && set->GetSoldItemCount() != 0) ||
			   (type == UnSold && set->GetUnsoldItemCount() != 0) ||
					...
					if(item)
					{
						   ...
						   (type == Original))
						{
							if((item->m_serial_number >= m_serials_from) &&
							   ((item->m_serial_number <= m_serials_to) || (m_serials_to == -1)))
							{
								SProductItemNode *newItem = NULL;
								SALIL_NEW(newItem, SProductItemNode);
								*newItem = *item;
								itemlist.AddHead(newItem);
							}
						}
					}
	...
	while(!itemlist.IsEmpty())
	{
		SProductItemNode *item = GetLowestSerialNumberItem(&itemlist);
		if(item)
			position = PhotoInvoice_AddItem(item, position);
	...
	FinalizeLandscapeReport(FALSE);
	progress2.FinishDialog();
	return TRUE;
}


void CRatnaKiranReports::PhotoInvoice_AddTitle(ReportType type)
{
	SFileHeaderNode *dbHeader = m_header;
	if(!dbHeader)
	{
	...
	if(type == Original)
		...
		FillCell(gAlpha[0], 1, "SOLD PHOTO INVOICE");
	else if(type == UnSold)
		FillCell(gAlpha[0], 1, "UNSOLD PHOTO INVOICE");
...
void CRatnaKiranReports::PhotoInvoice_AddHeaders()
{
	SFileHeaderNode *dbHeader = m_header;
	if(!dbHeader)
	{
	...
	MergeCells(gAlpha[column], position, gAlpha[column], position + 1);
	SetWidth(gAlpha[column], gAlpha[column], 30);
	FillCell(gAlpha[column++], position, "Picture");

	if(m_use_new_serials)
	{
		MergeCells(gAlpha[column], position, gAlpha[column], position + 1);
		SetWidth(gAlpha[column], gAlpha[column], 5);
		FillCell(gAlpha[column++], position, "Serial no.");
	}
	...
	{
		MergeCells(gAlpha[column], position, gAlpha[column], position + 1);
		SetWidth(gAlpha[column], gAlpha[column], 8);
		FillCell(gAlpha[column++], position, "Old Serial no.");
	}
	...
	MergeCells(gAlpha[column], position, gAlpha[column], position + 1);
	SetWidth(gAlpha[column], gAlpha[column], 12);
...
int CRatnaKiranReports::PhotoInvoice_AddItem(SProductItemNode *item, int position)
{
	if(!item)
		return position;

	if(m_use_new_serials && m_use_old_serials)
		SetHeight(position, position, (630 / m_image_count));
	else if(m_use_new_serials)
		SetHeight(position, position, (600 / m_image_count));
	else if(m_use_old_serials)
		SetHeight(position, position, (600 / m_image_count));
	else
		SetHeight(position, position, (575 / m_image_count));

	int index = 1;
	while(index <= m_last_column)
	{
		SetAlignment(gAlpha[index], position, CENTERALIGN, CENTERALIGN);
		index++;
	}
	...
	FillCell(gAlpha[column++], position, item->m_image_data, item->m_image_size);
	...
	FillCell(gAlpha[column++], position, "VS");
	FillCell(gAlpha[column++], position, item->GetDiamondWeight(), 2);
	FillCell(gAlpha[column++], position, item->GetStoneWeight(), 2);
	FillCell(gAlpha[column++], position, item->GetPearlWeight(), 2);
	FillCell(gAlpha[column++], position, total, 2);
...
BOOL CRatnaKiranReports::CreateExportChecklist()
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreateExportChecklist - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
	...
	while(!itemlist.IsEmpty())
	{
		SProductItemNode *item = GetLowestSerialNumberItem(&itemlist);
		if(item)
			position = ExportChecklist_AddItem(item, position);
...
void CRatnaKiranReports::ExportChecklist_AddTitle()
{
	SetBold(gAlpha[0], 1, gAlpha[m_last_column], 1);
	MergeCells(gAlpha[0], 1, gAlpha[m_last_column], 1);
	FillCell(gAlpha[0], 1, "CHECKLIST");
...
int CRatnaKiranReports::ExportChecklist_AddItem(SProductItemNode *item, int position)
{
	if(!item)
		return position;
...
BOOL CRatnaKiranReports::CreateExportTags(int row, int column)
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreateExportTags - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
			...
			while(index < set->GetItemCount())
			{
				...
				if(item)
				{
					if((item->m_serial_number >= m_serials_from) &&
					   ((m_serials_to == -1) || (item->m_serial_number <= m_serials_to)))
					{
						ExportTags_AddItem(item, (2 * itemnumber), position);
						itemnumber++;
...
void CRatnaKiranReports::ExportTags_AddItem(SProductItemNode *item, int column, int row)
{
	if(!item)
		return;
...

	{
		...
		if(m_use_old_serials)
		{
			...
			{
				temp.Format("%d",serial);
				firstside = firstside + "#" + temp + " = ";
			}
		}
			
		//PRODUCT ID
		firstside = firstside + item->m_productID + "\n";
...

	{
		...
		//PRODUCT ID
		secondside = secondside + item->m_productID + "\n";
...
BOOL CRatnaKiranReports::CreateExportPriceQuotation()
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreateExportPriceQuotation - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
	...
	ExportPriceQuotation_AddTitle();
	progress1.ProgressForward();

	int position = m_start_row;

	CList<SProductSetNode *, SProductSetNode *> setlist;
	while(!m_list->IsEmpty())
	{
		CString productID = m_list->RemoveHead();
		SProductSetNode *set = m_database->GetSet(productID);
	...
	{
		SProductSetNode *set = GetLowestSerialNumberSet(&setlist);
		if(set)
		{
#if USE_TWO_COLUMNS
			if(onfirstpage)
			...
			position = ExportPriceQuotation_AddSet(set, position, orientation);
...
void CRatnaKiranReports::ExportPriceQuotation_AddTitle()
{
	SetBold(gAlpha[0], 1, gAlpha[m_last_column], 1);
	MergeCells(gAlpha[0], 1, gAlpha[m_last_column], 1);
	FillCell(gAlpha[0], 1, "Price Quotation");
	SetAlignment(gAlpha[0], 1, CENTERALIGN, CENTERALIGN);
	SetFontSize(gAlpha[0], 1, gAlpha[m_last_column], 1, 10);
	SetUnderline(gAlpha[0], 1, gAlpha[m_last_column], 1);
}
...
void CRatnaKiranReports::ExportPriceQuotation_AddHeaders()
{
	...
	price_quotation_page_size = m_rows_in_a_page;
#endif
}


int CRatnaKiranReports::ExportPriceQuotation_AddSet(SProductSetNode *set, int position, int orientation)
{
	if(!set)
		return position;
	...
	while(index < set->GetItemCount())
	{
		...
		if(item)
		{
			if((item->m_serial_number >= m_serials_from) &&
			   ((item->m_serial_number <= m_serials_to) || (m_serials_to == -1)))
			{
				pos = ExportPriceQuotation_AddItem(item, pos, orientation);
			}
		}
...
int CRatnaKiranReports::ExportPriceQuotation_AddItem(SProductItemNode *item, int position, int orientation)
{
	if(!item)
		return position;

	double gold_rate, conversion, price;
	CalculateExportValues(&gold_rate, &conversion, NULL, NULL, NULL, NULL, &price, item);
...
BOOL CRatnaKiranReports::CreateGRForm()
{
	if(!m_database || !m_list || !m_valid)
	{
		MessageBox(NULL, "CRatnaKiranReports::CreateGRForm - conditions not met.", "ERROR", MB_OK);
		return FALSE;
	}
		...
		if(set)
		{
			...
			while(index < set->GetItemCount())
			{
				...
				if(item)
				{
					...
					((m_serials_to == -1) || (item->m_serial_number <= m_serials_to)))
					{
						GRForm_AddItem(item, &pieces, &gross_weight, &net_weight, &total_value);
					}
				}
	...
	GRForm_FillValues(pieces, gross_weight, net_weight, total_value);
	progress.ProgressForward();
...
void CRatnaKiranReports::GRForm_CreateOutline(char *startcolumn, int startrow, char *endcolumn, int endrow, BOOL hasEntry, BOOL drawBorder)
{
	if(hasEntry)
	{
		MergeCells(startcolumn, startrow, endcolumn, startrow);
...
void CRatnaKiranReports::GRForm_AddItem(SProductItemNode *item,
										int *pieces, double *gross_weight,
...
void CRatnaKiranReports::GRForm_FillValues(int pieces, double gross_weight, 
										   double net_weight, double total_value)
{
	SFileHeaderNode *dbHeader = m_header;
	if(!dbHeader)
	...
	//EXPORTER
	GRForm_CreateOutline("A", 3, "D", 9, TRUE, TRUE);
	FillCell("A", 3, "Exporter:");
	FillCell("A", 4, exporter);

	//INVOICE NUMBER	
	GRForm_CreateOutline("E", 3, "G", 5, TRUE, TRUE);
	FillCell("E", 3, "Invoice No. & Date");
	FillCell("E", 4, invoice_number + "\nDt. " + date);
	
	//SB No. & DATE
	GRForm_CreateOutline("H", 3, "K", 5, TRUE, TRUE);
	FillCell("H", 3, "SB No. & Date");

	//AR4
	GRForm_CreateOutline("E", 6, "G", 7, TRUE, TRUE);
	FillCell("E", 6, "AR4/AR4A No. & Date");

	//IMPORT EXPORT CODE No.
	GRForm_CreateOutline("H", 6, "K", 7, TRUE, TRUE);
	FillCell("H", 6, "Import/Export Code No.");

	//Cert. No. & Date
	GRForm_CreateOutline("E", 8, "G", 9, TRUE, TRUE);
	FillCell("E", 8, "Q/Cert No. & Date");

	//STATE OF ORIGIN OF GOODS
	GRForm_CreateOutline("H", 8, "K", 9, TRUE, TRUE);
	FillCell("H", 8, "State of origin of goods");

	//CONSIGNEE
	GRForm_CreateOutline("A", 10, "D", 16, TRUE, TRUE);
	FillCell("A", 10, "Consignee:");
	FillCell("A", 11, consignee);

	//EXPORT TRADE CONTROL
	GRForm_CreateOutline("E", 10, "G", 18, TRUE, TRUE);
	FillCell("E", 10, "Export Trade Control");

	//IF EXPORT
	GRForm_CreateOutline("H", 10, "K", 14, FALSE, TRUE);
	FillCell("H", 10, "If export under:\nDeffered credit  [   ]       Joint Ventures  [   ]\nRupee Credit  [   ]       Others  [   ]\nRBI Approval/CIR No. & Date");

	//TYPE OF SHIPPMENT
	GRForm_CreateOutline("H", 15, "K", 18, FALSE, TRUE);
	FillCell("H", 15, "Type of shippment\nConsignment export  [   ]\nOughtRIGHTALIGN sale  [   ]       Others  [   ]");

	//PRE-CARRIAGE
	GRForm_CreateOutline("A", 17, "B", 19, TRUE, TRUE);
	FillCell("A", 17, "Pre-Carriage");

	//PLACE OF RECEIPT BY PRE-CARRIER
	GRForm_CreateOutline("A", 20, "B", 22, TRUE, TRUE);
	FillCell("A", 20, "Place of Receipt");// by Pre-Carrier");

	//VESSEL/FLIGHT No.
	GRForm_CreateOutline("C", 17, "D", 18, TRUE, TRUE);
	FillCell("C", 17, "Vessel/Flight No.");

	//ROTATION No.
	GRForm_CreateOutline("C", 19, "D", 20, TRUE, TRUE);
	FillCell("C", 19, "Rotation No.");

	//PORT OF LOADING
	GRForm_CreateOutline("C", 21, "D", 22, TRUE, TRUE);
	FillCell("C", 21, "Port of loading");
	FillCell("C", 22, "NEW DELHI");

	//PORT OF LOADING
	GRForm_CreateOutline("E", 19, "K", 22, FALSE, TRUE);
	FillCell("E", 19, "Nature of contact:\nCIF   [   ]   /   CFR [   ]   /   FOB [   ]\nOthers");

	//PORT OF DISCHARGE
	GRForm_CreateOutline("A", 23, "B", 24, TRUE, TRUE);
	FillCell("A", 23, "Port of Discharge");
	FillCell("A", 24, country);

	//PORT OF DESTINATION
	GRForm_CreateOutline("C", 23, "D", 24, TRUE, TRUE);
	FillCell("C", 23, "Port of Destination");
	FillCell("C", 24, country);

	//EXCHANGE RATE
	GRForm_CreateOutline("E", 23, "G", 24, TRUE, TRUE);
	str.Format("%.02f",conversion);
	str = currency + " 1.00 = Rs. " + str;
	...
	//CURRENCY OF INVOICE
	GRForm_CreateOutline("H", 23, "K", 24, TRUE, TRUE);
	FillCell("H", 23, "Currency of Invoice");
	FillCell("H", 24, currency);

	//MARK AND NOs. CONTAINER
	GRForm_CreateOutline("A", 25, "B", 26, FALSE, FALSE);
	GRForm_CreateOutline("A", 27, "B", 30, FALSE, FALSE);
	FillCell("A", 25, "Marks & Nos.");

	//No. & KIND OF PKGs.
	GRForm_CreateOutline("C", 25, "D", 26, FALSE, FALSE);
	GRForm_CreateOutline("C", 27, "D", 30, FALSE, FALSE);
	FillCell("C", 25, "No. & kind of Pkgs.\nContainer No.");

	//GR FORM No.
	GRForm_CreateOutline("A", 32, "B", 33, TRUE, FALSE);
	FillCell("A", 32, "GR Form No.:");

	//No. OF PIECES
	GRForm_CreateOutline("A", 34, "B", 35, TRUE, FALSE);
	FillCell("A", 34, "No. Of Pcs.:");
	FillCell("A", 35, pieces);

	//NET WEIGHT
	GRForm_CreateOutline("A", 37, "B", 38, TRUE, FALSE);
	FillCell("A", 37, "Net Weight:");
	FillCell("A", 38, net_weight,3);

	//GROSS WEIGHT
	GRForm_CreateOutline("A", 39, "B", 40, TRUE, FALSE);
	FillCell("A", 39, "Gross Weight:");
	FillCell("A", 40, gross_weight,3);

	//STATISTICAL CODE
	GRForm_CreateOutline("E", 25, "G", 26, FALSE, FALSE);
	GRForm_CreateOutline("E", 27, "G", 27, FALSE, FALSE);
	FillCell("E", 25, "Statistical code & Desc. of goods & EXIM Scheme code where applicable.");

	//GOLD JEWELLERY
	GRForm_CreateOutline("E", 29, "G", 32, FALSE, FALSE);
	FillCell("E", 29, "Gold jewellery of .750 fineness set with pearls, precious & semi precious stones & diamonds cut & polished.");

	//PERSONAL CARRIAGE
	GRForm_CreateOutline("E", 34, "G", 40, FALSE, FALSE);
	...
	//TOTAL FOB IN WORDS
	GRForm_CreateOutline("A", 42, "K", 43, TRUE, TRUE);
	FillCell("A", 42, "Total FOB in Words:");

	//ANALYSIS OF EXPORT VALUE
	GRForm_CreateOutline("A", 44, "B", 44, FALSE, FALSE);
	...
	//FULL EXPORT VALUE
	GRForm_CreateOutline("H", 44, "K", 50, FALSE, TRUE);
	...
	//UNDERTAKING
	GRForm_CreateOutline("A", 51, "K", 56, FALSE, TRUE);
	...
	//DECLARATION
	GRForm_CreateOutline("A", 57, "K", 64, FALSE, TRUE);
